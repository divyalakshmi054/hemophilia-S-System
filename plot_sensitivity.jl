...
function calculate_x_axis_tick_positions(data_array;box_width=1.0)
    ...
    for col_index = 1:number_of_cols
        ...
        push!(x_axis_points,origin_point+0.25*box_width)
    end
...
function visualize(data_array,x_axis_ticks,y_axis_ticks,y_label_array)
    ...
    x_label_array = ["H", "H", "T", "T"]
    ax.set_xticklabels(x_label_array, rotation=0,fontsize=8)
    ...
    axis("square")
    tight_layout()
	PyPlot.savefig("./figs/sensitivity_array.pdf")
...
function reduce(path_to_sensitivity_array::String; epsilon::Float64=1e-2)
    ...
    for row_index = 1:NR
        ...
        # check -
        idx_check = findall(x->(x>0.0), row_data)
...
path_to_sensitivity_array = joinpath(pwd(),"data","Sensitivity_HEM_rFVIIa50.csv")

# execute -
(idx_p, p_label_array, rsa) = reduce(path_to_sensitivity_array; epsilon=1e-5)
